...
#   pass <- GET(url)
#   fromJSON(rawToChar(pass$content)) |> 
#     data.frame(data) |> 
#     slice_head(n = 3)
# 
# }

# new function that prints out NULL if the lat and long don't work
pass_times <- function(latitude, longitude) {
  url <- paste0("https://api.g7vrd.co.uk/v1/satellite-passes/25544/",
                latitude, "/", longitude, ".json")
  pass <- GET(url)
  pass_data <- fromJSON(rawToChar(pass$content))
  if (length(pass_data$passes) == 0) {
    return(NULL)
  }
  pass_data |>
  data.frame(data) |> 
...
passes <- pass_times(51.45, -2.5833)
passes

pass_times(capital$Latitude[2], capital$Longitude[2])

#map2
#x = lat, y = long, functionname (pass)
#use rbind, put lists into dataframe
...
```{r}
# this map function isnt working
map2(capital$Latitude, capital$Longitude, pass_times)
```
